...
struct GpioIn
{
    ...
    {
        std::cout << BOLDBLACK << "Gpio reads true" << RESET << std::endl;
        return true;
    }
};
...
TEST(SPI, TransmitCalled)
{
    ...
    EXPECT_CALL(mock, transmitreceive(_, _, 1))
...
TEST(SPI, ReadReg)
{
    ...
    const uint16_t expected_tx = 32768;
    ...
    EXPECT_CALL(mock, transmitreceive(_, _, 1))
        .WillOnce(Invoke([&](uint8_t *data_tx, uint8_t *data_rx, const uint8_t len)
            ...
            uint16_t* tx_16bit = (uint16_t*)data_tx;
            EXPECT_EQ(*tx_16bit, expected_tx);
...
TEST(SPI, WriteReg)
{
    ...
    const uint16_t expected_tx = 15359;
    const uint8_t fake_rx[] = {0x55, 0x66};
    ...
    EXPECT_CALL(mock, transmitreceive(_, _, 1))
        .WillOnce(Invoke([&](uint8_t *data_tx, uint8_t *data_rx, const uint8_t len)
                         {
            EXPECT_EQ(len, 1);
            auto data_tx_16bit = (uint16_t*)data_tx;
            EXPECT_EQ(*data_tx_16bit, expected_tx);
